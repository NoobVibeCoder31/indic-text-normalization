"""
Idempotency and round-trip tests for the Telugu grammars.
"""
...
DATA_DIR = Path(__file__).parent.parent / "data" / "te"
...
# Telugu letters U+0C05-U+0C39, vowel signs U+0C3E-U+0C4D, digits U+0C66-U+0C6F, ZWJ/ZWNJ,
...
class TestIdempotency:
    ...
    def test_tn_golden_outputs_stable(self, te_tn: Normalizer, spoken: str) -> None:
        """
        ...
        assert te_tn.normalize(spoken) == spoken

    @pytest.mark.parametrize("written", sorted(set(_golden_outputs("itn"))))
    def test_itn_golden_outputs_stable(self, te_itn: InverseNormalizer, written: str) -> None:
        """
        Every ITN golden output passes through ITN unchanged.
        """
        if written in KNOWN_ITN_FAILURES:
            pytest.xfail("documented non-idempotent form")
        assert te_itn.inverse_normalize(written) == written

    @pytest.mark.parametrize("text", sorted(_SYMBOL_PAIRS))
    def test_tn_symbol_pairs_idempotent(self, te_tn: Normalizer, text: str) -> None:
        """
        Every digit/letter/symbol pair (glued or spaced) is a TN fixed point after one pass.
        """
        once = te_tn.normalize(text)
        assert te_tn.normalize(once) == once

    @given(text=st.text(alphabet=_TELUGU_ALPHABET, max_size=30))
    @settings(
        max_examples=200, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    def test_tn_random_idempotent(self, te_tn: Normalizer, text: str) -> None:
        """
        TN is idempotent over random Telugu-and-digit strings.
        """
        once = te_tn.normalize(text)
        assert te_tn.normalize(once) == once
    ...
    def test_itn_random_idempotent(self, te_itn: InverseNormalizer, text: str) -> None:
        """
        ITN is idempotent over random Telugu-and-digit strings.
        """
        once = te_itn.inverse_normalize(text)
        assert te_itn.inverse_normalize(once) == once
...
class TestRoundTrip:
    ...
    def test_integers_round_trip(self, te_tn: Normalizer, te_itn: InverseNormalizer) -> None:
        """
        ...
        failures = [
            (n, spoken, back)
            for n in numbers
            for spoken in [te_tn.normalize(str(n))]
            for back in [te_itn.inverse_normalize(spoken)]
            if back != str(n)
        ]
        assert failures[:20] == []
    ...
    def test_itn_outputs_survive_tn(
        self, te_tn: Normalizer, te_itn: InverseNormalizer, written: str
    ) -> None:
        """
    ...
    def test_tn_no_silent_fallback(
        self, te_tn: Normalizer, text: str, caplog: pytest.LogCaptureFixture
    ) -> None:
        """
    ...
    def test_itn_no_silent_fallback(
        self, te_itn: InverseNormalizer, text: str, caplog: pytest.LogCaptureFixture
    ) -> None:
        """
        No golden ITN input is returned unchanged because tagging or verbalization failed.
        """
        with caplog.at_level("WARNING", logger="indic_text_normalization.core.engine"):
            te_itn.inverse_normalize(text)
        assert not [
            r for r in caplog.records if "Failed" in r.message or "No verbalization" in r.message
        ]
