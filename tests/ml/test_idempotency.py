"""
Idempotency and round-trip tests for the Malayalam grammars.
"""
...
# (മുതൽ ... വരെ for a range, ശതമാനം for a percentage, സമം for "="), so TN followed by ITN
# cannot restore them.
ROUND_TRIP_EXEMPT = {"10-20", "12.5%", "5 = 5", "a = b", "1 കിലോ", "1 ദിവസം"}
# A bare ഒന്ന് is as often the adverb "just" as the numeral, so ITN leaves it alone; the
# counting ഒരു before a count noun is also the article and is left alone too.
...
# Malayalam letters U+0C05-U+0C39, vowel signs U+0C3E-U+0C4D, digits U+0C66-U+0C6F, ZWJ/ZWNJ,
...
class TestIdempotency:
    ...
    def test_tn_golden_outputs_stable(self, ml_tn: Normalizer, spoken: str) -> None:
        """
        ...
        assert ml_tn.normalize(spoken) == spoken

    @pytest.mark.parametrize("written", sorted(set(_golden_outputs("itn"))))
    def test_itn_golden_outputs_stable(self, ml_itn: InverseNormalizer, written: str) -> None:
        """
        Every ITN golden output passes through ITN unchanged.
        """
        if written in KNOWN_ITN_FAILURES:
            pytest.xfail("documented non-idempotent form")
        assert ml_itn.inverse_normalize(written) == written

    @pytest.mark.parametrize("text", sorted(_SYMBOL_PAIRS))
    def test_tn_symbol_pairs_idempotent(self, ml_tn: Normalizer, text: str) -> None:
        """
        Every digit/letter/symbol pair (glued or spaced) is a TN fixed point after one pass.
        """
        once = ml_tn.normalize(text)
        assert ml_tn.normalize(once) == once

    @given(text=st.text(alphabet=_MALAYALAM_ALPHABET, max_size=30))
    @settings(
        max_examples=200, deadline=None, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    def test_tn_random_idempotent(self, ml_tn: Normalizer, text: str) -> None:
        """
        TN is idempotent over random Malayalam-and-digit strings.
        """
        once = ml_tn.normalize(text)
        assert ml_tn.normalize(once) == once
    ...
    def test_itn_random_idempotent(self, ml_itn: InverseNormalizer, text: str) -> None:
        """
        ITN is idempotent over random Malayalam-and-digit strings.
        """
        once = ml_itn.inverse_normalize(text)
        assert ml_itn.inverse_normalize(once) == once
...
class TestRoundTrip:
    ...
    def test_integers_round_trip(self, ml_tn: Normalizer, ml_itn: InverseNormalizer) -> None:
        """
        ...
        failures = [
            ...
            for spoken in [ml_tn.normalize(str(n))]
            for back in [ml_itn.inverse_normalize(spoken)]
            if back != str(n)
        ]
        assert failures[:20] == []
    ...
    def test_itn_outputs_survive_tn(
        self, ml_tn: Normalizer, ml_itn: InverseNormalizer, written: str
    ) -> None:
        """
        ...
        spoken = ml_tn.normalize(written)
        assert ml_itn.inverse_normalize(spoken) == written

    @pytest.mark.parametrize("text", sorted(set(_golden_inputs("tn"))))
    def test_tn_no_silent_fallback(
        self, ml_tn: Normalizer, text: str, caplog: pytest.LogCaptureFixture
    ) -> None:
        """
    ...
    def test_itn_no_silent_fallback(
        self, ml_itn: InverseNormalizer, text: str, caplog: pytest.LogCaptureFixture
    ) -> None:
        """
        No golden ITN input is returned unchanged because tagging or verbalization failed.
        """
        with caplog.at_level("WARNING", logger="indic_text_normalization.core.engine"):
            ml_itn.inverse_normalize(text)
        assert not [
            r for r in caplog.records if "Failed" in r.message or "No verbalization" in r.message
        ]
